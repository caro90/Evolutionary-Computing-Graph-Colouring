function graphColouringInitialization(datasetPath,numberOfVertices,numberOfColours,rng)
    ...
    colouringPartioning = Array{Array{Int64}}(numberOfColours)
    for i=1:numberOfColours
    ...
    for i=1:numberOfVertices
        j=j+1;
        if (j>numberOfColours)
            j = 1
        end
        append!(colouringPartioning[j],i)
    end
